...
def run_experiment():
    ...
    # 2. Compute its STFT representation
    ...
    print("Running STFT Fragility Experiment...")
    ...
    for amount in noise_amounts:
        ...
        # Measure similarity
        sim = cosine_similarity(clean_stft, noisy_stft)
    ...
    plt.plot(noise_amounts, similarities, marker='o', linewidth=2, color='#1f77b4')
    plt.title("STFT Fragility Under Additive Noise", fontsize=14, fontweight='bold', pad=15)
    ...
    # Save the graph
    output_path = os.path.join(project_root, "results", "exp001_stft_noise_fragility.png")
