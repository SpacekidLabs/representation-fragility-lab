...
def run_adaptive_tuner_prototype():
    print("=" * 60)
    print("RUNNING PROTOTYPE V1: ADAPTIVE TUNER (EXPERIMENT 019)")
    print("=" * 60)
    ...
    # 1. Synthesize Singing Voice: glide from 220 Hz (A3) to 440 Hz (A4) with 6 Hz vibrato
    print("Synthesizing vowel singing voice (portamento + vibrato)...")
    f_profile = np.zeros_like(t)
    # Portamento sweep
    f_profile = 220.0 + (440.0 - 220.0) * t / duration
    # Add 6 Hz vibrato with 15 Hz depth
    f_profile += 15.0 * np.sin(2 * np.pi * 6.0 * t)
    
    clean_sig = np.zeros_like(t)
    for k in range(1, 6):
        phases_k = 2 * np.pi * np.cumsum(k * f_profile) / sr
        amp_k = apply_formants(f_profile, phases_k, k)
        clean_sig += amp_k * np.sin(phases_k)
        
    # Apply envelope
    env = np.ones_like(t)
    attack_len = int(0.15 * sr)
    ...
    # Save clean source
    save_wav(os.path.join(project_root, "results", "audio", "source_clean.wav"), clean_sig, sr)
    
    # 2. Create the Vocal Conditions
    print("Generating vocal perturbation classes...")
    conditions = {}
    ...
    save_wav(os.path.join(project_root, "results", "audio", "source_noisy.wav"), conditions["noisy"], sr)
    
    # Filtered (lowpass at 600 Hz - removes overtones and second formant)
    ...
    save_wav(os.path.join(project_root, "results", "audio", "source_filtered.wav"), conditions["filtered"], sr)
    
    # Distorted (harsh hard-clipping)
    dist = np.clip(clean_sig, -0.2, 0.2)
    conditions["distorted"] = dist / np.max(np.abs(dist))
    save_wav(os.path.join(project_root, "results", "audio", "source_distorted.wav"), conditions["distorted"], sr)
    ...
    # We generate a fast training sweep
    training_frequencies = [261.63, 329.63, 392.00, 440.00] # C4, E4, G4, A4
    for i, f0 in enumerate(training_frequencies):
        # Generate clean harmonic stack with formants
        clean_tr = np.zeros_like(t[:int(sr * 1.5)])
        t_tr = t[:int(sr * 1.5)]
        for k in range(1, 6):
    ...
    # We evaluate 4 tuners
    tuners = ["acf", "stft", "hybrid", "meta"]
    
    # We will log the pitch tracks and meta weights for visualization on the clean/noisy signals
    ...
    for cond_name, audio_signal in conditions.items():
        print(f"\nProcessing vocal condition: {cond_name.upper()}...")
        ...
        for tuner_name in tuners:
            ...
            current_win = 4096 # Starting window size for adaptive routing
            ...
            for n in range(num_blocks):
                ...
                if tuner_name == "acf":
                    ...
                    # Reactive hybrid fusion (ACF + Cepstrum)
                    detected_pitch = hybrid_pitch_estimate([p_acf, p_cep], [c_acf, c_cep])
                elif tuner_name == "meta":
                    # Meta-Representation Layer Fusion (ACF + Cepstrum + STFT)
                ...
                # 2. Pitch correction mapping to nearest target note (Scale A major: A, B, C#, D, E, F#, G#)
                # A chromatic scale target is cleaner and simpler for a general prototype:
                # Target Note: nearest MIDI note
                s = 0.0
                if 80.0 <= detected_pitch <= 1000.0:
                    midi = int(np.round(12 * np.log2(detected_pitch / 440.0) + 69))
                    # Check target note frequency
                    f_target = 440.0 * (2.0 ** ((midi - 69) / 12.0))
                    # Semitone shift steps
                    s = 12.0 * np.log2(f_target / detected_pitch)
                    # Safe clip to avoid extreme stretching distortion
                    s = np.clip(s, -4.0, 4.0)
                    
                # 3. Apply phase-vocoder pitch shift to the block
                shifted_block = librosa.effects.pitch_shift(block, sr=sr, n_steps=s)
                
                # 4. Overlap-add with Hanning window
                win = np.hanning(win_size)
                output_audio[start:end] += shifted_block * win
                window_sum[start:end] += win
                
            # Normalize overlap-add
            mask_norm = window_sum > 1e-5
            output_audio[mask_norm] /= window_sum[mask_norm]
            
            # Truncate and normalize peak to 0.95
            ...
            out_path = os.path.join(project_root, "results", "audio", f"{cond_name}_{tuner_name}.wav")
            save_wav(out_path, output_audio, sr)
            
    # Convert logged tracks to arrays
    # Frame center timestamps
    t_frames = np.array([(n * hop_size + 4096) / sr for n in range(len(logged_tracks["clean"]["acf"]))])
    
    # -----------------------------------------------------------------
    # Step 5: Plotting the "Internal Brain" Exposer Panel
    # -----------------------------------------------------------------
    print("\nGenerating visual report and plot...")
    ...
    ax1 = axes[0]
    # True vocal pitch (reference portamento + vibrato)
    t_ref = np.linspace(0, duration, len(clean_sig))
    ax1.plot(t_ref, f_profile, color="#666666", linestyle="--", alpha=0.5, label="Source Pitch (Vibrato)")
    
    # Plot ACF vs STFT vs Meta pitch estimates under noise
    ...
    ax1.set_title("Pitch Detector Tracks under Noisy Vocal Condition (σ = 0.15)", fontsize=13, fontweight='bold')
    ...
    ax2.set_title("The Internal Brain: Dynamic Representation Weights Exposer (Noisy Vocal)", fontsize=13, fontweight='bold')
    ...
    ax3 = axes[2]
    # Target midi note pitches over time
    midi_target_p = np.array([440.0 * (2.0 ** ((int(np.round(12 * np.log2(f / 440.0) + 69)) - 69) / 12.0)) for f in f_profile])
    # Downsample targets to frames
    target_p_frames = np.array([midi_target_p[int(n * hop_size + 4096)] for n in range(len(logged_tracks["noisy"]["acf"]))])
    ...
    ax3.set_title("Pitch Tuning Deviation (Error to Target Notes) under Noise", fontsize=13, fontweight='bold')
    ...
    # Save Plot
    output_path = os.path.join(project_root, "results", "exp019_adaptive_tuner.png")
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print("-" * 60)
    print("PROTOTYPE V1 RUN SUCCESSFUL!")
    print("-" * 60)
    ...
    print("Mean Tuning Deviation to Target Notes (Noisy Vocal):")
